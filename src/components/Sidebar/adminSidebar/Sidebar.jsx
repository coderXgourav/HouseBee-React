...
const Sidebar = ({ children }) => {
  ...
  const isMenuActive = (menuItems) => {
    return menuItems.some((item) => isActive(item.link));
  };
  ...
  const menuChange = (changeMenu) => {
    if (expandedMenu === changeMenu) {
      // Only allow closing if the submenu isn't active
      if (!isMenuActive(submenus[changeMenu])) {
        setExpandedMenu(null);
      }
    } else {
      setExpandedMenu(changeMenu);
    }
    setMenu(changeMenu);
  };
  ...
  const submenus = {
    1: [
      { name: "Home Buyers", link: "/admin/home-buyers" },
      { name: "Realtor Leads", link: "/admin/realtors" },
      { name: "Loan", link: "/admin/loan" },
    ],
    2: [
      { name: "Home Owners", link: "/admin/home-owners" },
      { name: "Lenders", link: "/admin/lenders" },
      { name: "Realtor", link: "/admin/user-realtor" },
    ],
  };
  ...
  useEffect(() => {
    if (isActive("/admin/dashboard")) {
      setMenu(0);
      setExpandedMenu(null);
    } else if (isMenuActive(submenus[1])) {
      setMenu(1);
      setExpandedMenu(1); // Auto expand submenu
    } else if (isMenuActive(submenus[2])) {
      setMenu(2);
      setExpandedMenu(2); // Auto expand submenu
    } else if (isActive("/admin/setting")) {
      setMenu(3);
      setExpandedMenu(null);
    }
  }, [location.pathname]);
        ...
        <div>
          <div className="row">
            ...
            <div className="col-md-10">
              <div
                className={`bg p-3 ${isActive("/admin/dashboard") ? "active" : ""} ${menu === 0 ? "round" : ""}`}
                onClick={() => menuChange(0)}
              >
                <Link to={"/admin/dashboard"} className="sidebarLinks">
              ...
              <div className="sidebar-menu-container">
                <div
                  className={`bg p-3 ${
                    isSubmenuActive(submenus[1]) ? "active-parent" : menu === 1 ? "round" : ""
                  }`}
                  onClick={() => menuChange(1)}
                >
                  <div className="d-flex align-items-center justify-content-between">
                    <Link to={""} className="sidebarLinks">
                      <div className="d-flex align-items-center">Lead Management</div>
                    </Link>
                ...
                {expandedMenu === 1 && (
                  <div className="submenu-container ps-4">
                    {submenus[1].map((submenu, index) => (
                      <div
                        key={index}
                        className={`submenu-item py-2 ${isActive(submenu.link) ? "active-submenu" : ""}`}
                      >
                        <Link to={submenu.link} className="sidebarSubLinks">
              ...
              <div className="sidebar-menu-container">
                <div
                  className={`bg p-3 ${
                    isSubmenuActive(submenus[2]) ? "active-parent" : menu === 2 ? "round" : ""
                  }`}
                  onClick={() => menuChange(2)}
                >
                  <div className="d-flex align-items-center justify-content-between">
                    <Link to={""} className="sidebarLinks">
                      <div className="d-flex align-items-center">User Management</div>
                    </Link>
                    <i className={`text-secondary`}></i>
                  </div>
                </div>
              ...
              <div
                className={`bg p-3 ${isActive("/admin/setting") ? "active" : ""} ${menu === 3 ? "round" : ""}`}
                onClick={() => menuChange(3)}
              >
                <Link to={"/admin/setting"} className="sidebarLinks">
            ...
            <div className="col-md-12 bg-light border ">
              <div className="row header">
                <div className="col-md-4 userWelcome">
                  <p>Good Morning, Boris </p>
                  <span className=" userTestText">Here is your daily preview</span>
                </div>
